...
import { Bell, CheckCircle, ChevronRight, Cpu, Edit, Fingerprint, Home, Info, LogOut, Mail, MapPin, Shield, Smartphone, User as UserIcon } from "lucide-react-native";
import { useCallback, useState } from "react";
import { Image, ScrollView, Text, TouchableOpacity, View } from "react-native";
import { useSafeAreaInsets } from "react-native-safe-area-context";
import { useTheme } from "../../utils/themeContext";
import { UserData, useUser, userStore } from "../../utils/userStore";
            ...
            <ScrollView showsVerticalScrollIndicator={false} className="flex-1">
                ...
                <View className="px-6 mb-24">
                    ...
                    <View className={`${isDarkMode ? "bg-[#1A1A1A] border-[#333]" : "bg-white border-gray-100"} rounded-3xl border shadow-sm p-2`}>
                        ...
                        <TouchableOpacity onPress={() => route.push("/edit-profile")} className={`flex-row items-center p-4 gap-4 border-b ${isDarkMode ? "border-[#333]" : "border-gray-50"}`}>
                            <View className="w-10 h-10 rounded-2xl bg-[#FAEBE4] items-center justify-center">
                                <Mail size={20} color="#D66A1F" />
                            </View>
                            <View className="flex-1">
                                <Text className="text-[#888] text-xs" style={styles.text}>Email Address</Text>
                                <Text className={`text-base ${isDarkMode ? "text-white" : "text-[#333]"}`} style={styles.subtitle}>{user.email}</Text>
                            </View>
                            <Edit size={16} color="#CCC" />
                        </TouchableOpacity>
